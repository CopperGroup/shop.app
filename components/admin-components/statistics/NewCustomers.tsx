...
import { findNewCustomers } from "@/lib/actions/order.actions"
...
export function NewCustomers() {
  ...
  const [ totalNewCustomers, setTotalNewCustomers ] = React.useState(0);
  ...
  React.useEffect(() => {
    const fetchNewCustomers = async () => {
      const {data, overall} = await findNewCustomers(date?.from, date?.to);

      setData(data);
      setTotalNewCustomers(overall);
    }
    fetchNewCustomers();
  }, [date])
    ...
    <section className="w-full h-[25rem] mt-10">
      <div className="w-full h-full">
        <div className="w-full h-fit flex gap-2 justify-end max-[1300px]:flex-col">
          <div className="w-full h-full">
            <h3 className="text-heading3-bold font-semibold">Нові клієнти <span className="text-green-500 text-heading4-medium">+{totalNewCustomers}</span></h3>
          </div>
  ...
  if (active && payload && payload.length) {
    return (
      <div className={`bg-white/70 rounded-xl shadow-lg p-3`}>
        ...
        <p className="text-subtle-medium mt-1">Нові клієнти: <span className={`${payload[0].value > 0 && "text-green-500"}`}>+{payload[0].value}</span></p>
      </div>
    );
  }
  return null;
