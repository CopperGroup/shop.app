...
import { findCanceledOrders } from "@/lib/actions/order.actions"
...
export function CanceledOrders() {
  ...
  React.useEffect(() => {
    const fetchCanceledOrders = async () => {
      const {data, overall} = await findCanceledOrders(date?.from, date?.to);

      setData(data);
      setTotalCanceledOrders(overall);
    }
    fetchCanceledOrders();
  }, [date])
  ...
  return (
    <section className="w-full h-[25rem] mt-104">
      <div className="w-full h-full">
        <div className="w-full h-fit flex gap-2 justify-end max-[1300px]:flex-col">
          <div className="w-full h-full">
            <h3 className="text-heading3-bold font-semibold">Скасовані замовлення <span className={`${totalCanceledOrders > 0 ? "text-red-500" : "text-green-500"} text-heading4-medium`}>{totalCanceledOrders}</span></h3>
          </div>
  ...
  if (active && payload && payload.length) {
    return (
      <div className={`bg-white/70 rounded-xl shadow-lg p-3`}>
        ...
        <p className="text-subtle-medium mt-1">Скасованих замовлень: <span className={`${payload[0].value > 0 && "text-red-500"}`}>{payload[0].value}</span></p>
      </div>
    );
  }
  return null;
