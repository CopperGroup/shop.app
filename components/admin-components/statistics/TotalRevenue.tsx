...
export function TotalRevenue() {
  ...
  React.useEffect(() => {
    const fetchTotalRevenue = async () => {
      const receivedData = await findTotalRevenue(date?.from, date?.to);

      setData(receivedData);

      await findSalesByCategory(date?.from, date?.to);
    }
    fetchTotalRevenue();
  }, [date])
    ...
    <section className="w-full h-[25rem] mt-10">
      <div className="w-full h-full">
        <div className="w-full h-fit flex gap-2 justify-end">
          <div className="w-full h-full">
            <h3 className="text-heading3-bold font-semibold">Загальний дохід</h3>
          </div>
  ...
  if (active && payload && payload.length) {
    return (
      <div className={`bg-white/70 rounded-xl shadow-lg p-3`}>
        ...
        <p className="text-subtle-medium mt-1">Загальний дохід: <span className={`${payload[0].value > 0 && "text-green-500"}`}>₴{payload[0].value.toFixed(2)}</span></p>
      </div>
    );
  }
  return null;
