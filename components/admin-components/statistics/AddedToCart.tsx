...
import { findAddedToCart } from "@/lib/actions/order.actions"
...
export function AddedToCart() {
  ...
  const [ totalAddedToCart, setTotalAddedToCart ] = React.useState(0);
  const [ screenWidth, setScreenWidth ] = React.useState(0);


  React.useEffect(() => {
    const fetchAddedToCart = async () => {
      const {data, overall} = await findAddedToCart(date?.from, date?.to);

      setData(data);
      setTotalAddedToCart(overall);
    }
    fetchAddedToCart();
  }, [date])
    ...
    <section className="w-full h-[25rem] mt-10">
      <div className="w-full h-full">
        <div className="w-full h-fit flex gap-2 justify-end max-[1300px]:flex-col">
          <div className="w-full h-full">
            <h3 className="text-heading3-bold font-semibold">Додано в кошик <span className="text-green-500 text-heading4-medium">{totalAddedToCart}</span></h3>
          </div>
  ...
  if (active && payload && payload.length) {
    return (
      <div className={`bg-white/70 rounded-xl shadow-lg p-3`}>
        ...
        <p className="text-subtle-medium mt-1">Додоно в кошик: {payload[0].value}</p>
      </div>
    );
  }
  return null;
