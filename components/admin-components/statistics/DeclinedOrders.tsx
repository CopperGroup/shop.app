...
import { findDeclinedOrders } from "@/lib/actions/order.actions"
...
export function DeclinedOrders() {
  ...
  const [ totalDeclinedOrders, setTotalDeclinedOrders ] = React.useState(0);
  const [ screenWidth, setScreenWidth ] = React.useState(0);


  React.useEffect(() => {
    const fetchDeclinedOrders = async () => {
      const {data, overall} = await findDeclinedOrders(date?.from, date?.to);

      setData(data);
      setTotalDeclinedOrders(overall);
    }
    fetchDeclinedOrders();
  }, [date])
    ...
    <section className="w-full h-[25rem] mt-10 max-[1300px]:mt-24">
      <div className="w-full h-full">
        <div className="w-full h-fit flex gap-2 justify-end max-[1300px]:flex-col">
          <div className="w-full h-full">
            <h3 className="text-heading3-bold font-semibold">Оплата відхилена <span className={`${totalDeclinedOrders > 0 ? "text-red-500" : "text-green-500"} text-heading4-medium`}>{totalDeclinedOrders}</span></h3>
          </div>
  ...
  if (active && payload && payload.length) {
    return (
      <div className={`bg-white/70 rounded-xl shadow-lg p-3`}>
        ...
        <p className="text-subtle-medium mt-1">Оплата відхилена: <span className={`${payload[0].value > 0 && "text-red-500"}`}>{payload[0].value}</span></p>
      </div>
    );
  }
  return null;
